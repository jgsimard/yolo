...
def iou(boxes1, boxes2, scope='iou'):
    """
...
class YoloNetwork(object):
    ...
    def __init__(self, is_training=True):
        ...
        self.logits = self.build_network(is_training=is_training)
    ...
    def build_network(self, keep_prob=0.5, is_training=True, scope='yolo'):
        with tf.variable_scope(scope):
            with slim.arg_scope([slim.conv2d, slim.fully_connected],
                ...
                # Stage 1
                net = tf.pad(self.images, np.array([[0, 0], [3, 3], [3, 3], [0, 0]]), name='pad_1')
    ...
    def loss_layer(self, predicts, labels, scope='loss_layer'):
        ...
        '''
        with tf.variable_scope(scope):
            predict_classes = tf.reshape(predicts[:, :self.boundary_classes],
                                         [self.batch_size, self.cell_size, self.cell_size, self.num_class])
            predict_scales = tf.reshape(predicts[:, self.boundary_classes:self.boundary_scale],
                                        [self.batch_size, self.cell_size, self.cell_size, self.boxes_per_cell])
            predict_boxes = tf.reshape(predicts[:, self.boundary_scale:],
            ...
            predict_boxes_tran = tf.stack([(predict_boxes[:, :, :, :, 0] + offset) / self.cell_size,  #
                                           ...
                                           # because the y axis goes down in a pictue
                                           tf.square(predict_boxes[:, :, :, :, 2]),
                                           # box height and wigth variation are less important then absolute position
                                           tf.square(predict_boxes[:, :, :, :, 3])],
                                          axis=4)

            iou_predict_truth = iou(predict_boxes_tran, boxes)
            ...
            class_loss = self.class_scale * tf.reduce_mean(tf.reduce_sum(tf.square(class_delta), axis=[1, 2, 3]),
                                                           name='class_loss')

            # object_loss (error of predicting if an object is in a cell given that there is one)
            object_delta = object_mask * (predict_scales - iou_predict_truth)
            object_loss = self.object_scale * tf.reduce_mean(tf.reduce_sum(tf.square(object_delta), axis=[1, 2, 3]),
                                                             name='object_loss')
            ...
            no_object_loss = self.no_object_scale * tf.reduce_mean(
                tf.reduce_sum(tf.square(no_object_delta), axis=[1, 2, 3]), name='noobject_loss')

            # coord_loss
            coord_mask = tf.expand_dims(object_mask, 4)
            boxes_delta = coord_mask * (predict_boxes - boxes_tran)
            coord_loss = self.coord_scale * tf.reduce_mean(tf.reduce_sum(tf.square(boxes_delta), axis=[1, 2, 3, 4]),
                                                           name='coord_loss')
